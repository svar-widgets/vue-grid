<script setup>
defineOptions({ name: "DemoDragToGrid" });
import { ref } from "vue";
...
const rowStyle = row => (row.id === dropTargetId.value ? "wx-drop-target" : "");
...
function onGridDragOver(ev) {
	...
	dropTargetId.value = locateID(ev, "data-id");
}

function onGridDragLeave(ev) {
	// only clear when the cursor actually leaves the wrapper
	if (!ev.currentTarget.contains(ev.relatedTarget)) dropTargetId.value = null;
}
...
function onGridDrop(ev) {
	...
	api.value.exec(
		"add-row",
		dropTargetId.value != null ? { row, after: dropTargetId.value } : { row }
	);
	...
	<div class="demo">
		<div class="cards">
			...
			<div
				...
				draggable="true"
				role="listitem"
				@dragstart="ev => onCardDragStart(ev, card)"
			>
				<div class="card-name">
					{{ card.firstName }}
					{{ card.lastName }}
		...
		<div
			...
			@drop="onGridDrop"
		>
			<Grid ref="api" :data="data" :columns="columns" :row-style="rowStyle" />
		</div>
	</div>
</template>
...
/* class lands on grid-owned .wx-row, so pierce the scoping */
.grid-box :deep(.wx-row.wx-drop-target) {
